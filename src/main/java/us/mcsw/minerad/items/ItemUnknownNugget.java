...
public class ItemUnknownNugget extends ItemMR {

	IIcon background, foreground;

	public ItemUnknownNugget() {
		super("nuggetUnknown");
	}
	...
	public void addInformation(ItemStack it, EntityPlayer pl, List list, boolean n) {
		...
		list.add("Slightly unstable");
		list.add(EnumChatFormatting.RED + "+ " + ((ConfigMR.UNKNOWN_ELEMENT_RADIATION / 9 + 1) * it.stackSize)
				+ " RAD/s");
	}
	...
	public void onUpdate(ItemStack it, World w, Entity e, int m, boolean n) {
		if (!w.isRemote) {
			if (e instanceof EntityPlayer && RadProperties.get(e) != null) {
				...
				props.addRadiation((double) ((ConfigMR.UNKNOWN_ELEMENT_RADIATION / 9 + 1) * it.stackSize) / 20.0, false);
			}
		}
	...
	public void registerIcons(IIconRegister reg) {
		background = reg.registerIcon(TextureReference.RESOURCE_PREFIX + "unknownNuggetBackground");
		foreground = reg.registerIcon(TextureReference.RESOURCE_PREFIX + "unknownNuggetForeground");
	}
